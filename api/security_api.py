...
from blockchain_audit_logger import get_audit_logger, AuditEventType
from hedera_client import get_hedera_client
...
async def get_audit_logs(user_id: Optional[str] = None):
    ...
    logs = audit_logger.get_audit_trail(user_id)
...
async def export_audit_trail():
    ...
    result = audit_logger.export_audit_trail()
...
async def get_hedera_status():
    """Get Hedera connection status"""
    try:
        status = {
            'connected': True,
            'network': 'testnet',
            'operator_id': os.getenv('OPERATOR_ID', 'N/A'),
            'topic_id': str(audit_logger.topic_id) if audit_logger.topic_id else None,
            'audit_logs': len(audit_logger.audit_logs),
        }
...
app = FastAPI(
    title="AIxCyber - Security Layer API",
    description="Authentication & Authorization with Hedera Blockchain",
    version="1.0.0"
)
...
try:
    ...
    print(f"⚠️ Hedera initialization warning: {e}")
...
async def verify_kyc(request: KYCVerifyRequest):
    ...
    # Log to blockchain
    audit_logger.log_kyc_event(
        user_id=request.user_id,
        status=result.get('kyc_status'),
        details=result
    )
